...
export class IndexedDBHelper {
   ...
   *  await db.delete(storeName, 11)
   */
  public delete(storeName: string, primaryKey: IDBValidKey): Promise<undefined> {
    return new Promise((resolve, reject) => {
  ...
  /** 建表 */
  private createStore(store: IIndexedDBStore, db: IDBDatabase = this.db!): void {
  ...
  public async initRequestHandler(): Promise<IndexedDBHelper> {
    return new Promise((resolve) => {
      ...
      dbReq.onblocked = (event) => {
        this.throwError(IndexedDBHelper.name, 'IndexedDB数据库连接被阻止', event);
      };
